...
const districtStandings = ref<Map<number, DistrictStanding>>(new Map());
...
function zoomTo(lat: number, lng: number, zoom: number) {
...
function resetZoom() {
    ...
    map.flyTo(initialBounds.getCenter(), map.getBoundsZoom(initialBounds), {
        duration: 1,
    });
}
...
function creerMasque(data: CarteElectorale, annee: 2022 | 2026) {
    if (!map) {
        return;
    }
    ...
    if (!masque) {
        const quebec = union(featureCollection(data.features));
        ...
        masque = difference(featureCollection([monde, quebec])) ?? undefined;
...
function districtColor(districtNumber: number): string {
    const district = districtStandings.value.get(districtNumber);

    return partyColor(district?.party_color ?? null);
}
...
function appliquerCouleursCirconscriptions(): void {
    geoJsonLayer?.setStyle((feature) => {
        ...
        return {
            color: '#333333',
            weight: 1,
            fillColor: districtColor(Number(properties.CO_CEP)),
            fillOpacity: 0.6,
        };
    });
}
...
async function chargerCompositionDissolution(): Promise<void> {
    ...
    try {
        const response = await fetch('/api/assembly/dissolution', {
            headers: {
        ...
        partyStandings.value = data.parties.map((party) => ({
            name: party.name,
...
function afficherCarte(data: CarteElectorale, mode: MapMode) {
    ...
    geoJsonLayer = L.geoJSON(data, {
        ...
        onEachFeature(feature, layer) {
            const properties = feature.properties as CirconscriptionProperties;

            layer.bindTooltip(properties.NM_CEP);
...
function afficherDissolution() {
    afficherCarte(carte2022, 'dissolution');
}

function afficherCarte2022() {
    afficherCarte(carte2022, 2022);
}
...
onMounted(() => {
    ...
    const quebecBounds = L.latLngBounds([44.9, -79.8], [62.6, -57.0]);
    ...
    const masquePane = map.getPane('masque');
...
<template>
    <main class="map-page">
        ...
        <div class="city-links">
            <button @click="resetZoom">Québec au complet</button>

            <button @click="zoomTo(45.5019, -73.5674, 10)">Montréal</button>

            <button @click="zoomTo(46.8139, -71.208, 10)">Québec</button>

            <button @click="zoomTo(46.343, -72.543, 11)">Trois-Rivières</button>

            <button @click="zoomTo(45.4042, -71.8929, 11)">Sherbrooke</button>

            <button @click="zoomTo(47.38, -61.86, 9)">
